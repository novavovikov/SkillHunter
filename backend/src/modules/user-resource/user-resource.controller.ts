...
export class UserResourceController {
  ...
  async findResources(@Query('q') q: string, @UserData() user: User) {
    if (q) {
      const userResources = await this.userResourceService.findByTitleResource(
        user.id,
        q
      )
  ...
  async getResourceContent(
    ...
    const userResourceViewOnly = excludeFieldsFromObject(
      ['status'],
      userResource
    )
    return {
  ...
  async addResource(
    ...
    const createdResource = userSkill.userResources.find(
    ...
    const resourceSkillRelation = await this.skillService.addResourceToSkill(
      userSkill.skill.id,
      resource
    )
    ...
    if (userSkill.skill.name === WELCOME_SKILL_NAME && resource.accepted) {
