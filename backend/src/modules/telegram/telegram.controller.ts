...
export class TelegramController {
  ...
  async auth(@UserData() user: User, @Param('token') token: string) {
  ...
  sendEvent(@Param('method') method: string, @Body() params) {
  ...
  async selectSkillset(link: string, user: User) {
    if (isUrl(link)) {
  ...
  async messageHandler(message: TelegramMessageDto) {
    ...
    if (TELEGRAM_COMMANDS.includes(text)) {
      return this.telegramService.sendEvent('sendMessage', {
        chat_id: user.telegramId,
        text:
          'This command is under development. Send me a link, I will save in your skillset!',
      })
    }
  ...
  async selectSkill(skillsetId: number, user: User) {
  ...
  async addResource(skillId: number, user: User) {
    const userData = await this.getUserCache(user)
    const resource = (await this.resourceService.createByLink(
      userData.link
    )) as Resource
    ...
    const userResource = userSkill.userResources.find(
      userResource => userResource.resource.id === resource.id
    )
  ...
  getUserCache(user: User) {
    return (
      this.cacheService.get<UserData>(
        `${TELEGRAM_CACHE_KEY}:${user.telegramId}`
      ) || ({} as UserData)
    )
  }
